...
## Setup
The `docx` package is installed locally inside this skill's `node_modules` by `bootstrap.sh` (see the **Setup** section of `SKILL.md`). Run it once before executing any script:

```bash
sh "{skillDir}/bootstrap.sh"
```

When invoking your script from a workspace dir outside this skill, prefix `node` with `export NODE_PATH="{skillDir}/node_modules"` so `require('docx')` resolves to the pinned local copy.
